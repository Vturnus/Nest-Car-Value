...
describe("UsersController", () => {
  ...
  beforeEach(async () => {
    fakeUsersService = {
      findOne: (id: number) => {
        return Promise.resolve({ id, email: "test@test.com", password: "pass" } as User);
      },
      find: (email: string) => {
  ...
  it("findUser throws an error if user with id is not found", async (done) => {
  ...
  it("signIn update session object and returns user", async () => {
    ...
    const user = await controller.signin({ email: "test@test.com", password: "pass" }, session);
